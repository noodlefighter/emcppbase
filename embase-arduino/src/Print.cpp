...
{
    // log_output_fn(true,"write(const uint8_t * %s , size_t %d )\n",buffer,size);
...
{
    // log_output_fn(true,"write(const char *str):%s\n",str);
...
int Print::printf(const char *format, ...)
{
    ...
    int ret = vsnprintf(tmp_buf,sizeof(tmp_buf),format,ap);
    va_end(ap);
...
int Print::printf(const __FlashStringHelper *format, ...)
{
  ...
  return this->printf((const char *)format, ap);
}
